...
# Storing stock details using Dictionaries
unit_price = {}
description = {}
stock = {}
warehouse = {}
...
# Open stocks file stored in local system
details = open("C:\\Users\\madhuyen\\Downloads\\hjstocks.txt", "r")

# First line of the file is the total number of items
...
for i in range(0, no_items):
    ...
    x1, x2 = line.split("#")
    x1 = int(x1)
    warehouse.update({x1: x2})

for i in range(0, no_items):
    line = (details.readline()).rstrip("\n")
    x1, x2 = line.split("#")
    x1 = int(x1)
    location.update({x1: x2})
...
print("TC- Total cost of Ware house")
print("Q-Quit")
...
while (c != "q" or c != "Q"):
    ...
    if (c == "q" or c == "Q"):
        ...
        p_wh = input("Enter part Warehouse: ")
        p_loc = input("Enter part location: ")
        ...
        warehouse.update({p_no: p_wh})
        location.update({p_no: p_loc})
        if (p_stock > -1):
        ...
        print()
        print("Part number: ", p_no, " Description: ", description.get(p_no), " Price: ", unit_price.get(p_no),
              "WareHouse: ", warehouse.get(p_no), " Location: ", location.get(p_no)," Stock: ", stock.get(p_no))
    ...
    elif(c=="TC" or c=="tc"):   #Total Cost of WareHouse
        p_wh = input("Enter part Warehouse: ")
        p_no = [x for x,y in warehouse.items() if warehouse.get(x)==p_wh]
        print(p_no)
        total_price=0
        for i in range(0,len(p_no)):
            total_price += unit_price.get(p_no[i])*stock.get(p_no[i])
        print("Total Price of Warehouse ",p_wh," is: ",total_price)
        print()
...
details = open("C:\\Users\\madhuyen\\Downloads\\hjstocks.txt", "w")
...
for i in range(0, no_items):
    details.write(str(i + 1) + "#" + str(warehouse[i + 1]) + "\n")

for i in range(0, no_items):
    details.write(str(i + 1) + "#" + str(location[i + 1]) + "\n")
